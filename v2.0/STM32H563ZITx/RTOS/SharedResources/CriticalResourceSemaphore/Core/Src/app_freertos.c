...
/* Definitions for controlSemaphore */
osSemaphoreId_t controlSemaphoreHandle;
...
void AccessSharedData(void)
{
	static OccupationFlags occupationFlag = RESOURCE_FLAG_IDLE;

	if (occupationFlag == RESOURCE_FLAG_IDLE)
	{
		printf("Shared resource idle.\n");

		occupationFlag = RESOURCE_FLAG_OCCUPIED;

		/* Simulate the operation of Write/Read. */
		osDelay(500);

		occupationFlag = RESOURCE_FLAG_IDLE;
	}
	else
		printf("Shared resource occupied.\n");
}
