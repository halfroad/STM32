...
void StartFlashGreenLedTask(void *argument)
{
	...
	for (;;)
	{
		osSemaphoreAcquire(GreenLedEventBinarySemaphoreHandle, osWaitForever);
...
void StartFlashYellowLedTask(void *argument)
{
	...
	for (;;)
	{
		...
		{
			if ( i == 20)
				osSemaphoreRelease(GreenLedEventBinarySemaphoreHandle);
		...
		{
			if ( i == 10)
				osSemaphoreRelease(RedLedEventBinarySemaphoreHandle);
...
void StartFlashRedLedTask(void *argument)
{
	...
	for (;;)
	{
		osSemaphoreAcquire(RedLedEventBinarySemaphoreHandle, osWaitForever);
