...
/* Definitions for FlashRednLedTask */
osThreadId_t FlashRednLedTaskHandle;
const osThreadAttr_t FlashRednLedTask_attributes = {
  .name = "FlashRednLedTask",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
...
void StartFlashRednLedTask(void *argument);
...
void MX_FREERTOS_Init(void) {
  ...
  /* creation of FlashRednLedTask */
  FlashRednLedTaskHandle = osThreadNew(StartFlashRednLedTask, NULL, &FlashRednLedTask_attributes);
...
/* USER CODE BEGIN Header_StartFlashRednLedTask */
/**
 * @brief Function implementing the FlashRednLedTask thread.
 * @param argument: Not used
 * @retval None
 */
/* USER CODE END Header_StartFlashRednLedTask */
void StartFlashRednLedTask(void *argument)
{
  /* USER CODE BEGIN StartFlashRednLedTask */
	/* Infinite loop */
	for (;;)
	{
		osDelay(1);
	}
  /* USER CODE END StartFlashRednLedTask */
}
